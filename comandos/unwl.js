...
con.connect(function(err) {
    ...
    var sql = `UPDATE vrp_users SET whitelisted = '0' WHERE id = '${userid}'`;
    con.query(sql, function (err, result) {
        if (err) throw err;
        message.channel.send(result.affectedRows + ` ${message.author} - Você reprovou o passaporte: ${userid} com sucesso!`)
    })
})
}
